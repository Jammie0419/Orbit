...
### 后续增强（P2 反馈回路，超出本文档原案，建议后续实现）

> **状态**: 建议方案，未实现。本文档原案只承诺"记录路由历史"，不承诺"从历史学习"。
> 该增强由实现阶段的代码审计提出（2026-08-15），写入本文档以便后续排期。

**动机**：当前 `TOOL_SETS`、技能权重、harness 分支参数均为人工拍定的静态值。
`routing_history.jsonl` 已记录每次路由的 `task_type / branch / tools / skills`，
但没有"任务结果"和"模型实际使用了哪些工具"，因此无法从中学习。

**第一步：补齐记录（数据基础）**——路由历史中回填"实际使用"：
- 任务结束后，从 `tools.jsonl`（memory 记录的实际工具调用）回填：路由了 N 个，
  模型实际调用了哪几个。
- 关联任务终态（`task_done` 的 reason_code）作为"有效/无效"标签。

**三个学习闭环**：

1. **工具集自动瘦身（最直接）**
   统计每类任务历史上模型实际调用的工具 TOP-K，自动把 `TOOL_SETS[task_type]`
   瘦身到"实际高频"子集 → 信封更小、更省 token、数据驱动而非拍脑袋。

2. **技能权重自校准**
   统计推荐的技能里被 `skill_exec` 实际调用过的比率，上调高频、下调低频技能权重，
   甚至调整 `SKILL_TAG_MAPPING` → 技能推荐从"按相关性猜"进化到"按实际效用"。

3. **分支参数自调优**
   统计同类型任务在 `coding` 分支 vs `main` 兜底的成功率，强化有效分支、
   回退无收益分支 → harness 分支从"人工配置"进化到"按结果调优"。

**关键设计难点（为什么是 P2 而非 P1）**：
- 需要"任务结果"信号：历史记录目前无成功/失败，需接终态才有标签。
- 冷启动：每种任务需积累足够样本（约 20+ 条）才有统计意义。
- 防过拟合/防震荡：自动瘦身可能误删"低频但关键时刻有用"的工具，
  需保底（如保留所有被 `enable_tools` 主动开启过的工具）且变更可回滚。

**建议优先级**：先做闭环 1 + 记录补齐（收益最直接、最可量化）。
