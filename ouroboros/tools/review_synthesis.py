...
def build_plan_review_system_prompt(
    ...
    parts = [(
        ...
        "Implementation authorship (advisory): for each substantial implementation block — judged semantically as real coding work, never by a line/file count — the plan should name its author (a delegated child, or the root as integrator with a stated rationale such as harness unavailable, trivial delta, or salvage exhausted); flag as a RISK any plan whose substantial implementation stays with the root without that rationale.\n\n"
